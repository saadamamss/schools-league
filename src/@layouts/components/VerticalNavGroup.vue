<script setup lang="ts">
  ...
  const isAnyChildOpen = (children: any[]): boolean => {
  ...
  watch(() => route.path, () => {
    ...
    isGroupOpen.value = isActive && !isVerticalNavMini(windowWidth.value, isVerticalNavHovered.value).value
  ...
  watch(isVerticalNavMini(windowWidth.value, isVerticalNavHovered.value), val => {
...
<template>
  <li
    v-if="canViewNavMenuGroup(item as any)"
    class="nav-group"
    :class="[
      {
