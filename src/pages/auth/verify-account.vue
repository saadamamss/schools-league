...
const email = Cookies.get("verf-mail");
...
const submitOTP = async () => {
  try {
    ...
    if (response.token) {
      ...
      setTimeout(() => {
        router.replace("/");
        localStorage.removeItem("verf-mail-expiry");
      }, 1000);
    }
  } catch (error) {
    console.log(error);
...
const initCountdown = () => {
  ...
  const storedExpiry = localStorage.getItem("verf-mail-expiry");
  ...
  if (storedExpiry) {
    ...
    localStorage.setItem("verf-mail-expiry", newExpiry.toString());
  }
...
const resendOTP = async () => {
  try {
    ...
    const response = await auth.resendVerficationOTP(email);
