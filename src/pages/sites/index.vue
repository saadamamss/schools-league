...
const headers = [
  { title: "اسم الموقع", value: "site_name", align: "start" },
  { title: "نوع الموقع", value: "site_type" },
  { title: "رقم تصريح المسكن", value: "license" },
  { title: "رقم تواصل المفوض", value: "phone" },
  { title: "عدد الموظفين المرتبطين", value: "joined_employee" },
  { title: "ساعات العمل", value: "work_hours" },
  { title: "", value: "actions" },
];
...
const fetchSites = async () => {
  ...
  try {
    // ======= get real data =========
    //  const response = await axiosIns.get("sites" , {
    ...
    // ========= get fake data =========
    const response = await generatedData();
    if (response.data.data) {
      sites.value = response.data?.data;
      perPage.value = response.data?.meta?.perPage;
      totalItems.value = response.data?.meta?.totalItems;
...
const fetchSelectedSiteData = async (siteId) => {
  ...
  try {
    ...
    // }
    // ========= get fake data =========
    const response = await generatrSiteDetails(siteId);
    if (response.data?.data) {
    ...
    appStore.showSnackbar({
      message:
        err.response?.data?.message || "حدث خطأ فى جلب بيانات هذا الموقع ",
      color: "error",
    });
  } finally {
...
const generatedData = async () => {
  return await new Promise((resolve) => {
    ...
    setTimeout(() => {
      resolve({
        data: {
          data: sampleSites.slice(start, end),
          meta: {
            perPage: perPage.value,
            totalItems: sampleSites.length,
...
const generatrSiteDetails = async (id) => {
  return await new Promise((resolve) => {
    setTimeout(() => {
      const site = sites.value.find((i) => i.id == id);
      resolve({
        data: {
          data: {
            site_name: site.site_name,
            site_type: "مقر إدارى",
            license_no: "311254954",
            phone: "966555555555+",
            employee_number: "65",
            observer_number: "3",
            address: "شارع التخصصي، حي النخيل، الرياض",
            work_hours: "340",
            observers: [
              {
                name: "محمد بن عبد الله",
                role: "مشرف",
                work_hours: "340 ساعة هذا الشهر",
                last_checkin: "اليوم – 7:45 ص",
              },
...
const showDetails = (site) => {
  details_dialog.value = true;
  selectedsite.value = site;
  //
  fetchSelectedSiteData(site.id);
};
...
<template>
  <v-container fluid class="px-0">
    ...
    <v-container fluid class="px-0 px-sm-4 px-xl-0 py-0 mt-4">
        ...
        <VCardText>
          <v-row>
            <v-col cols="12">
              ...
              >
                <template #[`item.joined_employee`]="{ item }">
                  <span> {{ item.employee_number }} موظف </span> -
                  {{ item.observer_number }} مراقب
                </template>

                <template #[`item.work_hours`]="{ item }">
                  {{ item.work_hours }} ساعة هذا الشهر
                </template>
      ...
      <v-card rounded="lg">
        ...
        <v-card-title class="bg-app-light rounded-t-lg py-4">
          ...
          <div class="text-center">
            ...
            <h5 class="py-2">{{ selectedsite?.site_name }}</h5>
          </div>
        </v-card-title>
        ...
        <v-card-text class="pt-4 d-flex justify-center">
          ...
          <v-tabs v-model="tab" class="bg-app-light rounded-lg" hide-slider>
            <v-tab
              value="site-details"
              class="rounded-lg"
              :class="{ 'bg-success text-onSuccess': tab == 'site-details' }"
            >
              تفاصيل عن الموقع
            </v-tab>
            <v-tab
              value="observers"
              class="rounded-lg"
              :class="{ 'bg-success text-onSuccess': tab == 'observers' }"
            >
              المشرفون
            </v-tab>
            ...
            <v-tab
              value="employees"
              class="rounded-lg"
              :class="{ 'bg-success text-onSuccess': tab == 'employees' }"
            >
              الموظفون
            </v-tab>
          </v-tabs>
        ...
        <v-card-text>
          <v-window v-model="tab">
            <!-- Profile Tab -->
            <v-window-item value="site-details" class="px-3 py-3">
              <!-- Basic Info -->
              <VCard class="mb-4 px-2 border">
                <VCardTitle class="mb-1">
                  <h5>تفاصيل عن الموقع</h5>
                </VCardTitle>
                <v-divider></v-divider>
                  ...
                  <v-row class="mx-0 gap-2 px-2">
                    ...
                    <v-col cols="12" class="d-flex gap-2">
                      <span>
                        ...
                        >
                          <path
                            opacity="0.4"
                            d="M4.31726 0.0990676C4.88395 0.216192 5.33524 0.596931 5.61564 1.09996L6.50885 2.70241C6.83786 3.29261 7.11473 3.7893 7.29554 4.22107C7.48735 4.67908 7.60126 5.13074 7.54919 5.6301C7.49711 6.12946 7.29246 6.5479 7.01029 6.95649C6.74428 7.34167 6.37088 7.77054 5.92718 8.28017L4.61417 9.78835C4.37889 10.0586 4.26124 10.1937 4.25049 10.3626C4.23974 10.5315 4.33616 10.6758 4.529 10.9643C6.17259 13.4234 8.38773 15.6393 10.8488 17.2843C11.1374 17.4772 11.2816 17.5736 11.4505 17.5628C11.6194 17.5521 11.7546 17.4344 12.0248 17.1991L13.5331 15.8861C14.0427 15.4424 14.4716 15.069 14.8568 14.803C15.2653 14.5208 15.6838 14.3161 16.1831 14.2641C16.6825 14.212 17.1342 14.3259 17.5922 14.5177C18.0239 14.6985 18.5206 14.9754 19.1107 15.3043L19.1109 15.3044L20.7133 16.1976C21.2163 16.478 21.5971 16.9293 21.7142 17.496C21.8325 18.0686 21.658 18.6441 21.2724 19.1172C19.8735 20.8335 17.6322 21.9264 15.281 21.4521C13.8358 21.1605 12.4098 20.6748 10.6851 19.6857C7.2197 17.6983 4.11263 14.5895 2.12755 11.1282C1.13843 9.40349 0.652719 7.9775 0.361176 6.53224C-0.11312 4.18102 0.979715 1.93974 2.69608 0.540834C3.16911 0.155292 3.74466 -0.0192795 4.31726 0.0990676Z"
                            fill="rgba(var(--v-theme-primary))"
                          />
                          <path
                            fill-rule="evenodd"
                            clip-rule="evenodd"
                            d="M12.9999 0.8125C13.5522 0.8125 13.9999 1.26022 13.9999 1.8125V4.6125C13.9999 5.16478 13.5522 5.6125 12.9999 5.6125C12.4477 5.6125 11.9999 5.16478 11.9999 4.6125L11.9999 1.8125C11.9999 1.26022 12.4477 0.8125 12.9999 0.8125ZM18.6572 3.15617C19.0478 3.5467 19.0478 4.17986 18.6572 4.57039L16.6773 6.55029C16.2868 6.94081 15.6537 6.94081 15.2631 6.55029C14.8726 6.15976 14.8726 5.5266 15.2631 5.13607L17.243 3.15617C17.6336 2.76565 18.2667 2.76565 18.6572 3.15617ZM16.1999 8.8125C16.1999 8.26022 16.6477 7.8125 17.1999 7.8125L19.9999 7.8125C20.5522 7.8125 20.9999 8.26022 20.9999 8.8125C20.9999 9.36478 20.5522 9.8125 19.9999 9.8125H17.1999C16.6477 9.8125 16.1999 9.36478 16.1999 8.8125Z"
                            fill="rgba(var(--v-theme-primary))"
                          />
                        </svg>
                      </span>
                    ...
                    <v-col cols="12" class="d-flex gap-2">
                      ...
                      <div class="w-100">
                        ...
                        <div v-else class="text-dark">
                          {{ selectedsite?.employee_number }} موظف –
                          {{ selectedsite?.observer_number }} مراقبين
                        </div>
                      </div>
                    </v-col>
                    <v-col cols="12" class="d-flex gap-2">
            ...
            <!-- Attendance Tab -->
            <v-window-item value="observers" class="px-2 py-3">
              <v-card class="px-4 border">
                ...
                <VCardText class="px-0">
                  ...
                  <v-card
                    ...
                    v-for="observe in selectedsite?.observers"
                  >
                    <VCardTitle class="d-flex ga-2">
                      ...
                      <div class="d-flex flex-column">
                        <h5 class="mb-0">{{ observe.name }}</h5>
                        <span class="text-xs text-secondary" size="small">
                          {{ observe.role }}
                        </span>
                      </div>
                    </VCardTitle>
            ...
            <!-- Attendance Tab -->
            <v-window-item value="employees" class="px-2 py-3">
              <v-card class="px-4 border">
                <v-card-title class="px-0">
                  <h5>المشرفون</h5>
                </v-card-title>
                ...
                <VCardText class="px-0">
                  ...
                  <v-card
                    ...
                    rounded="lg"
                    class="border border-light mb-2"
                    v-for="observe in selectedsite?.employees"
                  >
                    <VCardTitle class="d-flex ga-2">
                      <span>
                        <User :size="45" />
