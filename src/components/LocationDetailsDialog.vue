<script setup lang="ts">
  ...
  import type { LocationDetail } from '@/types/models'
  ...
  const tab = ref('location-details')
  const selectedLocation = ref<LocationDetail | null>(null)
  const isLocationDetailsLoading = ref(false)

  const fetchselectedLocationData = async () => {
    isLocationDetailsLoading.value = true
    try {
      const response = await locationsApi.getById(props.locationId)
      const data = response.data?.data as unknown as LocationDetail | undefined
      if (data) {
  ...
  const openOnMap = () => {
    window.open(
      `https://maps.google.com/maps?q=${selectedLocation.value?.latitude},${selectedLocation.value?.longitude}`,
      '_blank'
    )
  }

  onMounted(() => { fetchselectedLocationData() })
</script>

<template>
  <v-dialog v-model="dialog" max-width="650" min-height="calc(100% - 40px)">
    <v-card class="d-block" rounded="lg">
      <!-- Header -->
      <v-card-title class="bg-light-gray border-b-md border-grey-900 rounded-t-lg py-4">
        <div class="w-100 d-flex justify-end" style="position: absolute; left: 10px; top: 10px">
        ...
        <div class="text-center">
          <v-avatar class="rounded-pill" color="light-success" :size="80">
            <v-img v-if="selectedLocation?.image" height="32" :src="selectedLocation?.image" width="32" />
            <svg
              v-else
              fill="none"
              height="34"
          ...
          <h5 class="py-2">{{ selectedLocation?.name }}</h5>
      ...
      <v-card-text class="pt-4 d-flex justify-center px-3">
        ...
        <v-tabs v-model="tab" class="rounded-lg" hide-slider>
          <v-tab
            ...
            :class="{
              'bg-light-success text-onSuccess': tab == 'location-details',
            }"
      ...
      <v-card-text class="px-0">
        <v-window v-model="tab">
          <!-- Profile Tab -->
          <v-window-item class="px-3 px-lg-6 py-3" value="location-details">
            <!-- Basic Info -->
            <VCard class="mb-4 px-0 border">
              ...
              <VCardText class="px-3 px-lg-6">
                <v-row class="mx-0 gap-2 px-0">
                  <v-col class="px-0 d-flex gap-2" cols="12">
                    <span>
                      <svg
                        fill="none"
                        height="26"
                    ...
                    <div class="w-100">
                      <div class="position-relative text-sec-text mb-2 d-flex align-center justify-space-between">
                      ...
                      <div v-if="isLocationDetailsLoading">
                        <v-skeleton-loader style="width: 100%; max-width: 200px" type="subtitle" />
                      </div>
                  ...
                  <v-col class="px-0 d-flex gap-2" cols="12">
                    ...
                    <div xlass="w-100">
                      ...
                      <div v-else class="text-primary-text">
                        {{ selectedLocation?.totalWorkingHours }} ساعة
                      </div>
                    </div>
                  </v-col>
                  <v-col class="px-0 d-flex gap-2" cols="12">
          ...
          <!-- observers Tab -->
          <v-window-item class="px-3 px-lg-6 py-3" value="observers">
            <v-card class="px-0 border">
              ...
              <VCardText class="px-3 px-lg-4">
                <v-skeleton-loader v-if="isLocationDetailsLoading" type="article" />

                <v-card v-if="selectedLocation?.supervisor" class="border border-light mb-2" rounded="lg">
                  <VCardTitle class="d-flex ga-2">
                    ...
                    <div class="d-flex flex-column">
                      <h5 class="mb-0">
                        {{ selectedLocation.supervisor.fullName }}
                      </h5>
                      <span class="text-xs text-sec-text" size="small">
                        {{ selectedLocation.supervisor.userType?.name?.ar }}
                      </span>
                    </div>
                  </VCardTitle>
                  <v-divider />
                    ...
                    <v-row class="mx-0">
                      <v-col class="d-flex gap-2" cols="12" sm="6">
                        ...
                        <div class="text-xs font-bold">
                          ...
                          <div class="text-primary-text">
                            {{ selectedLocation.supervisor.monthlyWorkingHours }}
                          </div>
                        </div>
                      </v-col>
                      <v-col class="d-flex gap-2" cols="12" sm="6">
                        ...
                        <div class="text-xs font-bold">
                          ...
                          <div class="text-primary-text">
                            {{ selectedLocation.supervisor.city?.name?.ar }}
                          </div>
                        </div>
                      </v-col>
                      <v-col class="d-flex gap-2" cols="12" sm="6">
                        ...
                        <div class="text-xs font-bold">
                          ...
                          <div class="text-primary-text">
                            {{
                              selectedLocation.supervisor.todayCheckIn
                      ...
                      <v-col class="d-flex gap-2" cols="12" sm="6">
                        ...
                        <div class="text-xs font-bold">
                          ...
                          <div class="text-primary-text">
                            {{
                              selectedLocation.supervisor.todayCheckOut
                                ? `اليوم - ${formatTimeTo12Hour(
                                  selectedLocation.supervisor.todayCheckOut
                                )}`
                                : "لم يسجل"
          ...
          <!-- employees Tab -->
          <v-window-item class="px-3 px-lg-6 py-3 align-start" value="employees">
            <v-card class="px-0 border">
              <v-card-title class="text-primary-text">
                <h5>الموظفون</h5>
              </v-card-title>
              ...
              <VCardText class="px-3 px-lg-4">
                <v-skeleton-loader v-if="isLocationDetailsLoading" type="article" />
                <v-card
                  v-for="(inspector, idx) in selectedLocation?.inspectors"
                  v-else-if="selectedLocation?.inspectors?.length"
                  :key="idx"
                ...
                >
                  <VCardTitle class="d-flex ga-2">
                    ...
                    <div class="d-flex flex-column">
                      <h5 class="mb-0">{{ inspector.fullName }}</h5>
                      <span class="text-xs text-sec-text" size="small">
                        {{ inspector.userType?.name?.ar }}
                      </span>
                    </div>
                  </VCardTitle>
                  <v-divider />
